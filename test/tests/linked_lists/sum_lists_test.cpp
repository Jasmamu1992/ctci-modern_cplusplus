...
TEST( SUM_LIST_RECURSIVE, EMPTY_LIST ) {
    Forward_List<int> list;
    node_pointer_int result = sum_lists_rec( list.front(), list.front());

    ASSERT_EQ( nullptr, result );
}

TEST( SUM_LIST_RECURSIVE, EQUAL_THREE_ELEMENTS ) {
    std::array<int, 3> input1{6, 1, 7};
    Forward_List<int> list1;
    for ( auto& num : input1 ) {
        list1.push_front( num );
    ...
    node_pointer_int result   = sum_lists_rec( list1.front(), list2.front());
    std::array<int, 3> output = {2, 1, 9};
    auto head                 = result;
    for ( size_t i = 0; i < output.size(); ++i ) {
        ASSERT_EQ( output[i], head->value_ );
...
TEST( SUM_LIST_RECURSIVE, INPUT1_LESS_SIZE ) {
    std::array<int, 2> input1{6, 1};
    Forward_List<int> list1;
    for ( auto& num : input1 ) {
        list1.push_front( num );
...
TEST( SUM_LIST_RECURSIVE, INPUT2_LESS_SIZE ) {
    std::array<int, 3> input1{2, 9, 5};
    Forward_List<int> list1;
    for ( auto& num : input1 ) {
        list1.push_front( num );
